...
import CancelModal from "./../assets/images/cross.png";
import PieChart from "../components/dashboard/PieChart";
import Breadcrumbs from "../components/default/SecondNavBar";
import {
  fetchExpenseReport,
  fetchLast4MonthsReport,
...
import Next from "../assets/images/dropdown.png";
export default function Report() {
  ...
  chartRefs.current = last4MonthsReport.map(
    (_, i) => chartRefs.current[i] ?? createRef()
  );
  ...
  if (currentIndex === 3 && prevBtnRef.current)
  ...
  const nextChartHandler = () => {
    ...
    chartRefs.current[
      previousIndex
    ].current.style.left = `calc(35% + ${currentGap}px)`;
                        ...
                        >
                          ...
                          >
                            <span className="tooltiptext">{`$${
                              expense
                                ? formatTotalAmount(expense.total_amount)
                                : 0
                            }`}</span>
                          </td>
                          <td
                          ...
                          >
                            <span className="tooltiptext">{`$${
                              income
                                ? formatTotalAmount(income.total_amount)
                                : 0
                            }`}</span>
                          </td>
                        </tr>
          ...
          <div className="dashboard-right">
            <div className="dashboard-right-top">
              <div className="dashboard-right-top-container">
                ...
                <div>
                  <span className="font-size-28 mt-2">
                    Budget ${expenseReports.budget}
                  </span>
                  <span
                ...
                <div className="font-size-28">
                  Expense ${expenseReports.total_expense}
                </div>
                <div className="font-size-28">
                  Remainder ${expenseReports.reminder}
                </div>
              </div>
            </div>
